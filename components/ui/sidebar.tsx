...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "ml-3 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border pl-2.5 py-0.5",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
    {...props}
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-state="closed"
      className={cn(
        "absolute right-1 top-1 lg:opacity-0 lg:group-hover/menu-sub-item:opacity-100 bg-background border flex aspect-square w-5 items-center justify-center p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 peer-hover/menu-button:text-sidebar-accent-foreground [&>svg]:size-4 [&>svg]:shrink-0",
        // Increases the hit area of the button on mobile.
        "after:absolute after:-inset-2 after:md:hidden",
        "peer-data-[size=sm]/menu-button:top-1",
        "peer-data-[size=default]/menu-button:top-1.5",
